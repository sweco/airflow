...
class KubeConfig:
    ...
    def __init__(self, prefix=""):
        multi_executor_conf = MultiExecutorAirflowConfigParser(conf, self.kubernetes_section)
        kubernetes_section = prefix + self.kubernetes_section

        configuration_dict = multi_executor_conf.as_dict(display_sensitive=True)
        self.core_configuration = configuration_dict[self.core_section]
        self.airflow_home = AIRFLOW_HOME
        self.dags_folder = multi_executor_conf.get(self.core_section, "dags_folder")
        self.parallelism = multi_executor_conf.getint(self.core_section, "parallelism")
        self.pod_template_file = multi_executor_conf.get(kubernetes_section, "pod_template_file", fallback=None)

        self.delete_worker_pods = multi_executor_conf.getboolean(kubernetes_section, "delete_worker_pods")
        self.delete_worker_pods_on_failure = multi_executor_conf.getboolean(
            kubernetes_section, "delete_worker_pods_on_failure"
        )
        self.worker_pod_pending_fatal_container_state_reasons = []
        if multi_executor_conf.get(kubernetes_section, "worker_pod_pending_fatal_container_state_reasons", fallback=""):
            self.worker_pod_pending_fatal_container_state_reasons = multi_executor_conf.get(
                kubernetes_section, "worker_pod_pending_fatal_container_state_reasons"
            ).split(",")

        self.worker_pods_creation_batch_size = multi_executor_conf.getint(
            kubernetes_section, "worker_pods_creation_batch_size"
        )
        self.worker_container_repository = multi_executor_conf.get(kubernetes_section, "worker_container_repository")
        self.worker_container_tag = multi_executor_conf.get(kubernetes_section, "worker_container_tag")
        ...
        self.kube_namespace = multi_executor_conf.get(kubernetes_section, "namespace")
        self.multi_namespace_mode = multi_executor_conf.getboolean(kubernetes_section, "multi_namespace_mode")
        if self.multi_namespace_mode and multi_executor_conf.get(
            kubernetes_section, "multi_namespace_mode_namespace_list"
        ):
            self.multi_namespace_mode_namespace_list = multi_executor_conf.get(
                kubernetes_section, "multi_namespace_mode_namespace_list"
            ).split(",")
        else:
        ...
        self.executor_namespace = multi_executor_conf.get(kubernetes_section, "namespace")

        self.worker_pods_queued_check_interval = multi_executor_conf.getint(
            kubernetes_section, "worker_pods_queued_check_interval"
        )

        self.kube_client_request_args = multi_executor_conf.getjson(
        ...
        self.delete_option_kwargs = multi_executor_conf.getjson(kubernetes_section, "delete_option_kwargs", fallback={})
